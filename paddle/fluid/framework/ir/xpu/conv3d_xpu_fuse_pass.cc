...
struct Conv3dXPUPattern : public PatternBase {
  Conv3dXPUPattern(PDPattern* pattern,
...
Conv3dXPUPattern::Conv3dXPUPattern(PDPattern* pattern,
                                   const std::string& name_scope,
                                   const std::string& conv_type,
                                   const std::string& act_type,
                                  //  const std::string& data_format,
  ...
  auto input = pattern->NewNode(input_repr())
                   ...
                   ->assert_more([](Node* node) {
                     return node->Var()->GetShape().size() == 5; // note here
                   });
...
/*
fuse conv3d block to xpu_conv3d op
For example:
graph[1]: sub block
                    in_Input
                      |
...
After the pass is applied:
       ...
       in_Filter      |     in_FilterMax
                 \    |    /
                  \   |   /
  in_Branch ------- __xpu__conv3d ------ in_Bias
                       |    \
                       |     \
                       |      out_OutputMax
                    out_Output
*/
class Conv3dXPUFusePass : public FusePassBase {
 protected:
 ...
 private:
  ...
  const std::string name_scope_{"conv3d_xpu_fuse_pass"};
};

void Conv3dXPUFusePass::ApplyImpl(ir::Graph* graph) const {
  PADDLE_ENFORCE_NOT_NULL(
      graph, platform::errors::PreconditionNotMet("graph should not be null."));
  Init(name_scope_, graph);

  int found_subgraph_count = 0;
  for (auto conv_type : {"conv3d",}) {
    for (auto data_format : {"NCDHW", "NDHWC"}) {
...
int Conv3dXPUFusePass::ApplyImpl(ir::Graph* graph,
                                 const std::string& conv_type,
                                 const std::string& act_type,
                                 const std::string& data_format,
                                 bool with_conv_bias,
  ...
  patterns::Conv3dXPUPattern pattern(gpd.mutable_pattern(),
                                     name_scope_,
                                     conv_type,
                                     act_type,
                                    //  data_format,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
                     Graph* graph) {
    VLOG(4) << "handle Conv3dXPUFusePass fuse";
    /* declare operator node's name */
    GET_IR_NODE(conv);
    ...
    // recompute bias and weight for conv3d_xpu op
    auto* filter_t =
    ...
    int out_channels = data_format == "NCDHW" ? filter_dims[0] : filter_dims[3];
    ...
    if (has_bias) {
      if (with_conv_bias) {
        ...
        PADDLE_ENFORCE_EQ(out_channels, // note here
    ...
    // output && output max
    std::string conv3d_xpu_out_name;
    if (!act_type.empty()) {
      conv3d_xpu_out_name = act_out->Name();
    } else if (ew_branch_add) {
      conv3d_xpu_out_name = ew_branch_add_out->Name();
    } else if (scale) {
      conv3d_xpu_out_name = scale_out->Name();
    } else if (bn) {
      conv3d_xpu_out_name = bn_out->Name();
    } else if (ew_bias_add) {
      conv3d_xpu_out_name = ew_bias_add_out->Name();
    } else {
      conv3d_xpu_out_name = conv_out->Name();
    }
    std::string conv3d_xpu_out_max_name = conv3d_xpu_out_name + "_max";
    VarDesc conv3d_xpu_out_max_desc(conv3d_xpu_out_max_name);
    Node* conv3d_xpu_out_max = graph->CreateVarNode(&conv3d_xpu_out_max_desc);
    // Generate conv3d_xpu op
    framework::OpDesc conv3d_xpu_op_desc(block);
    // set input&output var
    conv3d_xpu_op_desc.SetType("conv3d_xpu");
    conv3d_xpu_op_desc.SetInput("x", {input->Name()});
    conv3d_xpu_op_desc.SetInput("filter", {filter_int16->Name()});
    conv3d_xpu_op_desc.SetInput("filter_max", {filter_max->Name()});
    conv3d_xpu_op_desc.SetOutput("out", {conv3d_xpu_out_name});
    conv3d_xpu_op_desc.SetOutput("out_max", {conv3d_xpu_out_max_name});
    // set fusion_bias input node
    if (has_bias) {
      conv3d_xpu_op_desc.SetInput("bias", {fusion_bias_node->Name()});
    }
    // set ew_branch_add input node
    if (ew_branch_add != nullptr) {
      conv3d_xpu_op_desc.SetInput("branch", {ew_branch_add_in->Name()});
    }
    // set attrs of conv3d_xpu
    ...
    conv3d_xpu_op_desc.SetAttr("data_format", data_format);
    conv3d_xpu_op_desc.SetAttr("act_type", ConvertActivationType(act_type));
    conv3d_xpu_op_desc.SetAttr("act_param", act_param_);
    conv3d_xpu_op_desc.SetAttr(
        "padding_algorithm",
        conv->Op()->GetAttrIfExists<std::string>("padding_algorithm"));
    auto conv_paddings =
        PADDLE_GET_CONST(std::vector<int>, conv->Op()->GetAttr("paddings"));
    // reference: https://www.paddlepaddle.org.cn/documentation/docs/zh/api/paddle/nn/Conv3D_cn.html#conv3d
    if (conv_paddings.size() == 3) { // note here, padding size maybe 3
      int padding_depth = *(conv_paddings.begin() + 0);
      int padding_height = *(conv_paddings.begin() + 1);
      int padding_width = *(conv_paddings.begin() + 2);
      conv_paddings.insert(conv_paddings.begin()+1, padding_depth);
      conv_paddings.insert(conv_paddings.begin()+3, padding_height);
      conv_paddings.insert(conv_paddings.begin()+5, padding_width);
    }
    PADDLE_ENFORCE_EQ(conv_paddings.size(),
                      6UL,
                      platform::errors::InvalidArgument(
                          "padding length should be 6, but received %d, ",
                          conv_paddings.size()));
    conv3d_xpu_op_desc.SetAttr(
        "dilations",
    ...
    conv3d_xpu_op_desc.SetAttr("paddings", conv_paddings);
    conv3d_xpu_op_desc.SetAttr("out_dtype", out_dtype);

    auto* conv3d_xpu = graph->CreateOpNode(&conv3d_xpu_op_desc);
    IR_NODE_LINK_TO(input, conv3d_xpu);
    IR_NODE_LINK_TO(filter_int16, conv3d_xpu);
    IR_NODE_LINK_TO(filter_max, conv3d_xpu);
    if (ew_bias_add || bn) {
      SAFE_IR_NODE_LINK_TO(fusion_bias_node, conv3d_xpu);
    }
    if (ew_branch_add_in) {
      IR_NODE_LINK_TO(ew_branch_add_in, conv3d_xpu);
    }
    if (act_out) {
      IR_NODE_LINK_TO(conv3d_xpu, act_out);
    } else if (ew_branch_add_out) {
      IR_NODE_LINK_TO(conv3d_xpu, ew_branch_add_out);
    } else if (scale_out) {
      IR_NODE_LINK_TO(conv3d_xpu, scale_out);
    } else if (bn_out) {
      IR_NODE_LINK_TO(conv3d_xpu, bn_out);
    } else if (ew_bias_add_out) {
      IR_NODE_LINK_TO(conv3d_xpu, ew_bias_add_out);
    } else {
      IR_NODE_LINK_TO(conv3d_xpu, conv_out);
    }
    IR_NODE_LINK_TO(conv3d_xpu, conv3d_xpu_out_max);
    // delete useless node
...
REGISTER_PASS(conv3d_xpu_fuse_pass, paddle::framework::ir::Conv3dXPUFusePass);

REGISTER_PASS_CAPABILITY(conv3d_xpu_fuse_pass)
    .AddCombination(
